    ...
    public static Parser defaultParser() {
        ...
        synchronized (Compiler.class) {
            ...
            defaultParser = new LRParser.Builder("source")
                    ...
                    .define("literal-expression:string-expression", units -> units[0].value())
                    .define("literal-expression:integer-expression", units -> units[0].value())
                    .define("literal-expression:decimal-expression", units -> units[0].value())
                    .define("string-expression:normal-string", units -> new StringExpression(units[0].value()))
                    .define("string-expression:raw-string", units -> new StringExpression(units[0].value()))
                    .define("integer-expression:integer", units -> new IntegerExpression(units[0].value()))
                    .define("decimal-expression:decimal", units -> new DecimalExpression(units[0].value()))
